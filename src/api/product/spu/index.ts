...
export const reqSpuData = (
  ...
  request.get<any, SpuDataResponse>(
    API.GET_SPUDATA_URL + `${page}/${limit}?category3Id=${category3Id}`,
  );
// 请求获取品牌列表
export const reqSpuTrademarks = () =>
  request.get<any, SpuTrademarksResponse>(API.GET_SPUTRADEMARKS_URL);
// 请求获取SPU图标列表
export const reqSpuImages = (spuId: number) =>
  request.get<any, SpuImagesResponse>(API.GET_SPUIMAGES_URL + spuId);
// 请求获取SPU销售属性列表
export const reqSpuAttrs = () =>
  request.get<any, SpuAttrsResponse>(API.GET_SPUATTRS_URL);
// 请求获取SPU已有销售属性值列表
export const reqSpuAttrValues = (spuId: number) =>
  request.get<any, SpuAttrValuesResponse>(API.GET_SPUATTRVALUES_URL + spuId);
...
// 添加SKU
export const reqAddSku = (skuData: SkuData) =>
  request.post<any, any>(API.ADD_SKU_URL, skuData);
// 查找SKU
export const reqGetSkus = (spuId: number) =>
  request.get<any, SkuDataResponse>(API.GET_SKU_URL + spuId);

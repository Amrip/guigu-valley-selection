<template>
    <el-form label-width="100px">
        ...
        <el-form-item label="平台属性">
            <el-form :inline="true">
                <el-form-item v-for="(item, index) in platformAttrList" :key="item.id" :label="item.attrName" style="margin:0 10px">
                    <el-select v-model="item.attrIdAndValueId">
                        <el-option v-for="(platformAttrValue, index) in item.attrValueList" :key="platformAttrValue.id" :label="platformAttrValue.valueName" :value="`${item.id}:${platformAttrValue.id}`"></el-option>
                    </el-select>
                </el-form-item>
            </el-form>
        </el-form-item>
        ...
        <el-form-item label="销售属性">
            <el-form :inline="true">
                <el-form-item v-for="(item, index) in saleAttrList" :key="item.id" :label="item.saleAttrName" style="margin:0 10px">
                    <el-select v-model="item.attrIdAndValueId">
        ...
        <el-form-item label="图片展示">
            <el-table border :data="imgList" ref="imgShowTable">
                ...
                <el-table-column label="操作">
                    <template #="{row,$index}">
                        <el-button type="primary" size="small" @click="setDefaultImg(row)">设为默认</el-button>
                    </template>
                </el-table-column>
            </el-table>
        </el-form-item>
...
import {reqAddSku, reqSpuAttrValues, reqSpuImages} from '@/api/product/spu/index'
import {reqAttrInfo} from '@/api/product/attr/index'
import {AttrInfoResponseDate} from '@/api/product/attr/type'
import { SkuData,SpuAttrValuesResponse, SpuDataRecord, SpuImagesResponse } from '@/api/product/spu/type';
import { ref } from 'vue';
...
let initAddSku = async(row:SpuDataRecord)=>{
    ...
    // 发请求拿数据
    // 获取平台属性
    let result1:AttrInfoResponseDate = await reqAttrInfo(categoryStore.c1Id,categoryStore.c2Id,categoryStore.c3Id)
    // 获取销售属性

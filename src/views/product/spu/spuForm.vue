  ...
  <el-form label-width="100px">
    ...
    <el-form-item label="SPU品牌">
      <el-select v-model="spuParams.tmId">
        <el-option
          v-for="(item, _index) in spuTrademarkList"
          :key="item.id"
          :value="item.id"
          :label="item.tmName"
        ></el-option>
    ...
    <el-form-item label="SPU销售属性">
      <!-- 下拉属性框 -->
      <el-select
        v-model="spuSelectAttr"
        :placeholder="
          unSelectAttr.length ? `还有${unSelectAttr.length}个未选择` : '无'
        "
      >
        <el-option
          v-for="(item, _index) in unSelectAttr"
          :key="item.id"
      ...
      <el-button
        ...
        @click="addAttr"
        style="margin: 0 10px"
        :disabled="spuSelectAttr ? false : true"
        >添加属性</el-button
      >
      <!-- table展示属性 -->
        ...
        <el-table-column label="操作" width="120px">
          <template #="{ row, $index }">
            <el-button
              ...
              size="small"
              icon="Delete"
              @click="spuAttrValueList.splice($index, 1)"
            ></el-button>
          </template>
        </el-table-column>
      </el-table>
...
const initSpuData = async (spuBaseData: SpuDataRecord) => {
  ...
  // 发送请求获取已有的销售属性值列表
  let result4: SpuAttrValuesResponse = await reqSpuAttrValues(
    spuBaseData.id as number,
  );

  spuTrademarkList.value = result1.data;
  spuImageList.value = result2.data.map((item: any) => {
    // 这里要转换成elementPlus需要的格式
    return {
      name: item.imgName,
      url: item.imgUrl,
    };
